...
std::unique_ptr<cudaq::CompileTarget>
getDefaultPythonCompileTarget(sample_policy &) {
...
std::unique_ptr<cudaq::CompileTarget>
quantum_platform::getCompileTarget(ExecutionContext *ctx, std::size_t qpu_id) {
  validateQpuId(qpu_id);
  auto &qpu = platformQPUs[qpu_id];
  return qpu->getCompileTarget(ctx);
}

std::unique_ptr<cudaq::CompileTarget>
quantum_platform::getCompileTarget(sample_policy &policy, std::size_t qpu_id) {
  validateQpuId(qpu_id);
  auto &qpu = platformQPUs[qpu_id];
